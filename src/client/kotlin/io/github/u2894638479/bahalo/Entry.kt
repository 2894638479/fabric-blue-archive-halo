...
object Entry: DslEntryClient, DslEntryGui, ModMenuApi {
    ...
    override fun initializeClient() {
        ...
        WorldRenderEvents.AFTER_ENTITIES.register {
            val matrix = it.matrixStack() ?: return@register
            matrix.push()
            val camera = it.camera().pos
            matrix.translate(-camera.x,-camera.y,-camera.z)
            ClientCacheBeaconsRenderer.render(it.world().time,it.tickCounter().getTickDelta(false),matrix,it.consumers() ?: return@register)
            matrix.pop()
        }
    }
