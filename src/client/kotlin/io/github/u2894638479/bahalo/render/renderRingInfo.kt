...
fun renderRingInfo(
    ...
    stack {
        ms.mulPose(Axis.YP.rotation(PI.toFloat() - yaw.toFloat()))
        ms.mulPose(Axis.XP.rotation(-pitch.toFloat()))
        ms.translate(0.0, height, 0.0)

        stack {
            val sides = if(ringInfo.autoSide) run {
                val currentMatrix = ms.last().pose()
            ...
            ms.mulPose(Axis.YP.rotation(rotation.toFloat()))
        ...
        ringInfo.subRings.forEach {
            stack {
                ...
                ms.mulPose(Axis.YP.rotation(rotation1.toFloat()))
