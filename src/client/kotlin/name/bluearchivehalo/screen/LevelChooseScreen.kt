...
class LevelChooseScreen(parent: Screen) : MyScreen(
    Text.translatable("text.fabric-blue-archive-halo.title.level_choose"), parent
) {
    override fun init() {
        ...
        for (level in 1..16) {
            val button = ButtonWidget.builder(
                Text.translatable(
                    "text.fabric-blue-archive-halo.button.beacon_level",
                    level,
                    ringCount(level)
                )
            ) {
            ...
            // Подсказка для уровней выше 4 (можно добавить новый ключ в lang файл)
            if (level > 4) {
                button.tooltip(
                    Text.translatable(
                        "text.fabric-blue-archive-halo.tooltip.non_vanilla_level"
                    ).string
                )
            }
